...
class WriteToFile:
    def __init__(self, filename, *args):
        ...
        '5': 'generated/{}.xml'.format(_given_args[0]),
        '6': 'generated/tags/{}/index.html'.format(_given_args[0]),
        ...
        for root, dirs, files in os.walk('.'):
            for name in files:
                if name.endswith('.html'):
                    if root.startswith('./post'):
                        time_of_last_mod = os.path.getmtime((os.getcwd()\
                            .replace('generated', '') + 'text posts/') + os.path.split(root)[1])
                    else:
                        time_of_last_mod = os.path.getmtime(name)
                    strftime_last_mod = time.strftime("%Y-%m-%d", 
                            time.localtime(time_of_last_mod))
                    found_file = os.path.join(root, name)\
                        .replace('./', self.cfg.get('important', 'address'))
                    url_loc_lastmod += '''\n<url>
    <loc>{0}</loc>
    <lastmod>{1}</lastmod>
...
<rss version="2.0">
    ...
    def start_pagination(self):
        ...
        while self.one_dict:
            ...
            with open(pagination_dir + 'index.html', 'wt') as bambam:
                bambam.write(curr_header + posts_as_body + footer_pagi\
                    .replace("<!-- pagination -->",
                    ...
                    .format(newer=(address if not page_number > 1
                        else address + 'page/{}/'.format(page_number-1)),
    ...
    def __init__(self):
        ...
        header = ' '.join(format(ReadStaticFile("static/{}/header".format(template)))\
        .replace("site_description", self.cfg.get('important', 'description'))\
        .replace("titlefixed", self.cfg.get('important', 'name')).split())

        hf_post = ' '.join(format(ReadStaticFile("static/{}/header_post".format(template)))\
        .replace("site_description", self.cfg.get('important', 'description')).split())

        self.hf_pagination = ' '.join(format(ReadStaticFile("static/{}/header_pagination".format(template)))\
        .replace("site_description", self.cfg.get('important', 'description'))\
        .replace("titlefixed", self.cfg.get('important', 'name')).split())

        forum_header = ' '.join(format(ReadStaticFile("static/{}/header_forum".format(template)))\
        .replace("site_description", self.cfg.get('important', 'description')).split())

        search_header = ' '.join(format(ReadStaticFile("static/{}/header_search".format(template)))\
        .replace("site_description", self.cfg.get('important', 'description')).split())

        archive_header = ' '.join(format(ReadStaticFile("static/{}/header_archive".format(template)))\
        .replace("site_description", self.cfg.get('important', 'description'))\
        .replace("titlefixed", self.cfg.get('customizations', 'archive page title')).split())

        footer_p = ' '.join(format(ReadStaticFile("static/{}/footer_post".format(template)))\
        .replace("footer_text", self.cfg.get('customizations', 'footer text'))\
        .replace("blogfy_", self.cfg.get('social', 'disqus shortname')).split())

        footer_i = ' '.join(format(ReadStaticFile("static/{}/footer".format(template)))\
        .replace("footer_text", self.cfg.get('customizations', 'footer text')).split())

        self.footer_i_two = ' '.join(format(ReadStaticFile("static/{}/footer".format(template)))\
        .replace("footer_text", self.cfg.get('customizations', 'footer text')).split())
