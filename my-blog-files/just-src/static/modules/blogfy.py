...
class GenerateBlog(object):
    ...
    def start_atom(self):
        ...
        # old -> 'T%T+01:00'
        the_time   = strftime(self.date + 'T12:%M:00+01:00', localtime())
    ...
    def __init__(self):
        ...
        def replace_template_comments(template_part, var, dots=str()):
            return ' '.join(template_part.replace('<!-- whoami -->', var)
                ...
                .replace('<!-- ct -->', (self.gett('contact').replace('<!-- web_addr -->', self.address)
                                                               if var == 'Contact' else str())).split())
        dot_2              = '../../'
        top                = template_body[0]
        ...
        self.footer_p      = self.footer_p.replace('<!-- comment_section -->',
                (self.gett('intensedebate') if f_to_gen('disqus comments') else str()))#\
        ...
        self.open_and_write(path_join('generated', 'css', 'js', 'scr.js'),
            self.gett('tags_recent').format(self.recent_posts, self.tags_body) + self.gett('os_script'))
